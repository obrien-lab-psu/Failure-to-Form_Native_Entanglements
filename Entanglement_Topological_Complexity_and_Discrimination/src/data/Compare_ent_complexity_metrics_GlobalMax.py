...
class DataAnalysis:
    ...
    def __init__(self, outpath, uent_files, buff, spa, num_permute):
        ...
        'min_C_thread_depth_right', 'min_C_thread_slippage_right', 'ACO', 'RCO', 'prot_size']
    ...
    def load_files(self, mask):
        ...
        for gene in mask:
            ...
            if num_uent == 1:
                ...
                gene_uent['gene'] = gene
                gene_uent['num_uent'] = num_uent
                dfs += [gene_uent]

            if num_uent > 1:
                ## if the number of unique entanglements is greater than 1 take the maximum of each columns absolute value and make a single unique entanglement df for the gene
                #print(f'More than 1 uent for {gene}:\n{gene_uent}')
                gene_uent = gene_uent[self.keys]
                gene_uent = gene_uent.abs()
                gene_uent = gene_uent.max()
                gene_uent = gene_uent.to_frame().T
                gene_uent['gene'] = gene
                gene_uent['num_uent'] = num_uent
                dfs += [gene_uent]
                #print(f'gene_uent:\n{gene_uent}')
...
def main():
    ...
    Ess_stats = Analyzer.DistStats(Ess_ent_data, Analyzer.keys + ['num_uent'], 'Ess', n_resamples=num_permute)
    print(f'Ess_stats:\n{Ess_stats}')
    ...
    NonEss_stats = Analyzer.DistStats(NonEss_ent_data, Analyzer.keys + ['num_uent'], 'NonEss', n_resamples=num_permute)
    print(f'NonEss_stats:\n{NonEss_stats}')

    ## Compare the entanglement complexity between Essential and Nonessential data
    MWU_EssVNonEss_pvalues = Analyzer.Permutation(Ess_ent_data, NonEss_ent_data, Analyzer.keys + ['num_uent'], n_resamples=num_permute, type='MWU')
    print(f'MWU_EssVNonEss_pvalues:\n{MWU_EssVNonEss_pvalues}')

    Perm_EssVNonEss_pvalues = Analyzer.Permutation(Ess_ent_data, NonEss_ent_data, Analyzer.keys + ['num_uent'], n_resamples=num_permute, type='Perm')
