...
class DataAnalysis:
    ...
    def SignifantRescue(self, data, GT_regression, essential='essential', num_permutes=10000):
        ...
        for p in range(num_permutes):
            ...
            table = pd.crosstab(p_C['cuts'], p_C['region'])
            #print(f'Table:\n{table}')
            if table.values.shape == (2,2):
                C_OR, pvalue = fisher_exact(table) 
                #print(f'Fisher exact C: OR {C_OR} with pvalue {pvalue}')

            table = pd.crosstab(p_CD['cuts'], p_CD['region'])
            #print(f'Table:\n{table}')
            if table.values.shape == (2,2):
                CD_OR, pvalue = fisher_exact(table) 
                #print(f'Fisher exact CD: OR {CD_OR} with pvalue {pvalue}')
            ...
            table = pd.crosstab(p_CG['cuts'], p_CG['region'])
            #print(f'Table:\n{table}')
            if table.values.shape == (2,2):
                CG_OR, pvalue = fisher_exact(table) 
                #print(f'Fisher exact CG: OR {CG_OR} with pvalue {pvalue}')
    ...
    def run(self):
        ...
        print(f'FISHER {self.tag}| Essential: C2CD_pvalue: {Ess_C2CD_pvalue} | C2CG_pvalue: {Ess_C2CG_pvalue}')

        NonEss_C2CD_pvalue, NonEss_C2CG_pvalue = self.SignifantRescue(self.data, GT_regression, essential='nonessential')
        print(f'FISHER {self.tag}| NonEssential: C2CD_pvalue: {NonEss_C2CD_pvalue} | C2CG_pvalue: {NonEss_C2CG_pvalue}')
