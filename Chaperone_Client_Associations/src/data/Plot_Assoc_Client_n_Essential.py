...
class Plotter:
    ...
    def plot_data(self,):
        ...
        fig, axes = plt.subplots(3, 3, figsize=(8, 6), gridspec_kw={"height_ratios" : [5,5,5], "width_ratios" : [5,5,5]})
        ...
        for label, label_df in self.data.groupby('label'):
            for buff_i, buff in enumerate(['C', 'CD', 'CG']):
                ...
                if buff_i == 2:
                    axes[0, buff_i].errorbar(x, OR, yerr=[OR_lb_delta, OR_ub_delta], marker='o', ls='none', fillstyle='none', capsize=3, label=label)
                    axes[0, buff_i].legend(loc='upper left', bbox_to_anchor=(1,1))
                else:
                    axes[0, buff_i].errorbar(x, OR, yerr=[OR_lb_delta, OR_ub_delta], marker='o', ls='none', fillstyle='none', capsize=3, label=label)
                axes[0, buff_i].set_title(buff_tag[buff])
                axes[0, buff_i].axhline(y=1, color='red', linestyle='--')

                axes[1, buff_i].plot(x, pvalues, label='pvalue', marker='o', ls='none', fillstyle='none')
                axes[1, buff_i].axhline(y=0.05, color='red', linestyle='--')

                axes[2, buff_i].plot(x, n, label='pvalue', marker='o', ls='none', fillstyle='none')
