                ...
                for cov in np.arange(0,100,10):
                    for gene_tag in gene_tags:
                        ...
                        if buff == 'C' and set_type == 'EXP':
                            ...
                            #../../../git_slugs/Failure-to-Form_Native_Entanglements_slug/Modeling_Odds_of_Misfolding/Propensity_Score_Matching/AF/matched_residue_features_res_sasa_n1.csv
                            script = f'python src/data/Regression.py'
                            res_feats = f'-f ../../../git_slugs/Failure-to-Form_Native_Entanglements_slug/Modeling_Odds_of_Misfolding/Propensity_Score_Matching/{set_type}/matched_residue_features_res_sasa_n1.csv'
                            outpath = f'-o {path_to_slug}/Modeling_Odds_of_Misfolding/Regressions/{set_type}/PSM/'
                            gene_list = f'-g {path_to_slug}/Make_Protein_Feature_Files/Gene_lists/{set_type}/{set_type}_0.6g_{buff}_{timepoint}_spa{spa}_LiPMScov{cov}_{gene_tag}.txt'
                            misc = f'-t {gene_tag} -b {buff} -s {spa} -c {cov} -r "cut_{buff}_{timepoint} ~ AA + region" -l False -v region'
                            log = f'> {path_to_slug}/Modeling_Odds_of_Misfolding/Regressions/logs/{set_type}_PSM_{buff}_{timepoint}_spa{spa}_LiPMScov{cov}_{gene_tag}_region.log'

                            PSM_cmd = ' '.join([script, res_feats, outpath, gene_list, misc, log])
                            print(PSM_cmd)
