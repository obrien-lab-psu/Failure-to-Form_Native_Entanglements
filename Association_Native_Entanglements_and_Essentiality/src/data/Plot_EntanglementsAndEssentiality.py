...
class DataLoader:
    ...
    def load_data(self):
        ...
        print(self.data)
        outfile_csv = os.path.join(self.outpath, f'entanglement_and_essential_plot_data_{self.tag}.csv')
...
class Plotter:
    ...
    def plot_data(self,):
        ...
        outfile = os.path.join(self.outpath, f'entanglement_and_essential_Fisher_results_{self.tag}.png')

        #fig, axes = plt.subplots(1, 3, figsize=(8, 6), gridspec_kw={"height_ratios" : [5], "width_ratios" : [5,5,5]})
        fig, axes = plt.subplots(1, 3, figsize=(12, 4))

        for label, label_df in self.data.groupby('label'):
            for buff_i, buff in enumerate(['C']):
                buff_df = label_df[label_df['buff'] == buff]
                print(buff_i, buff,  buff_df)
                ...
                axes[1].plot(x, pvalues, label='pvalue', marker='o', ls='none', fillstyle='none')
        ...
        plt.suptitle(f'Assoc. Ent & Essential | {self.tag} | {buff_tag[buff]}')
        plt.tight_layout()
        #plt.show()
        plt.savefig(outfile)
        plt.close()
