...
class Analyzer:
    ...
    def __init__(self, outpath, all_gene_list, ent_gene_list, ess_gene_list, tag, buff, spa, LiPMScov):
        """
        ...
        print(f'Loaded ess_genes: {len(ess_genes)}')
        

        ## make initial dataframe for all genes 
        df = {'gene':[], 'entangled':[], 'essential':[]}
    ...
    ##################################################################
    def Fisher(self,):
        print(f'Calculating the OR and pvalue for the association of entangled genes with being essential')
        outfile = f'{self.Outpath}Fisher_stats_{self.tag}_{self.buff}_spa{self.spa}_LiPMScov{self.LiPMScov}.csv'
        print(f'outfile: {outfile}')

        reps = 10000
        data = {'OR':[], 'OR_lb':[], 'OR_ub':[], 'pvalue':[], 'tag':[self.tag], 'buff':[self.buff], 'spa':[self.spa], 'LiPMScov':[self.LiPMScov], 'entN_essentialN':[], 'entY_essentialN':[], 'entN_essentialY':[], 'entY_essentialY':[], 'total_res':[], 'n':[len(self.df)]}

        table = pd.crosstab(self.df['essential'], self.df['entangled'])
        print(f'ctable:\n{table}')
        if table.values.shape != (2,2):
            ...
            quit()
        entN_essentialN = table.values[0,0]
        entY_essentialN = table.values[0,1]
        entN_essentialY = table.values[1,0]
        entY_essentialY = table.values[1,1]
        data['entN_essentialN'] += [entN_essentialN]
        data['entY_essentialN'] += [entY_essentialN]
        data['entN_essentialY'] += [entN_essentialY]
        data['entY_essentialY'] += [entY_essentialY]
        data['total_res'] += [entN_essentialN + entY_essentialN + entN_essentialY + entY_essentialY]
        ...
        for b in range(reps):
            ...
            b_table = pd.crosstab(boot_df['essential'], boot_df['entangled'])
...
def main():
    ...
    parser.add_argument("-Esg", "--ess_gene_list", type=str, required=True, help=f"path to essential gene list to use")
    ...
    analysis = Analyzer(outpath, all_gene_list, ent_gene_list, ess_gene_list, tag, buff, spa, LiPMScov)
